...
u32 max_file_size = 0x7FFFFF; // 16.777 MB
u8 *file;
...
XStatus init(Demo *p_demo_inst) {
	...
	XGpio_SetDataDirection(&(p_demo_inst->gpio_out_inst), 1, 0x0000); // RGB LED
	...
	if (status != XST_SUCCESS) return XST_FAILURE;
	XGpio_SetDataDirection(&(p_demo_inst->gpio_in_inst), 1, 0xFF); // BUTTON it was 0x00
...
void dma_sw_tone_gen(Demo *p_demo_inst) {
	...
	float f = 2000,time_step, time=0;	//1.a	//the buffer is on 32 bits (enters in axis2fifo module).But
	double Y=0.0;						//1.a	//the data that I will send to the FIFO generator is on 16 bits.
	//	u8 val=0x00;					//1.b	//So I need to wait until the two adjacent elements buffer[i] and [i+1]
												//are filled with data. The data stored in buffer will be later read by AUDPWM
												//from fifo on 32 bits. The fifo_read_data represents the
...
//****************************************************************************************************************
	 ...
	 if (sample % 4 == 0) {   					//we put each sample in a temporary array on the
												//index that corresponds to the duty_reg that will
		((u8*)temp)[0] = Y;}
	 ...
	 {
		((u8*)temp)[3] = Y; 			//after 4 samples are observed, they can be stored on the
		 	 	 	 	 	 	 	 	//two buffer postions. The buffer will send their data to the
		 	 	 	 	 	 	 	 	//axis2fifo and fifoGenerator modules, where the data will be
		 	 	 	 	 	 	 	 	//concatenated.
		 	 	 	 	 	 	 	 	...
		 	 	 	 	 	 	 	 	// 03020100. PWM will start with LSB first (little endianes).
...
void recv_wav (Demo *p_demo_inst,u8 *file) {
	...
	uart_recv(p_demo_inst, file+sizeof(Wav_HeaderRaw), var); //these two are not included. Minus 4 because you don't want to include
